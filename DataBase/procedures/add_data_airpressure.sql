CREATE DEFINER=`myski`@`%` PROCEDURE `add_data_airpressure`(
...
set @creation:=CONCAT('UPDATE devices SET airpressure = '
					"JSON_SET(airpressure, '$.value', ",newData,", '$.time', '",timeS,"') "
					'WHERE iddevices = ',deviceID,';');
PREPARE stmt from @creation;
EXECUTE stmt;

#Also add the data to the temperature data table
INSERT INTO airpressure(iddevices, pressure, time) VALUES(deviceID, newData, timeS);
