CREATE DEFINER=`myski`@`%` PROCEDURE `add_data_humidity`(
...
set @creation:=CONCAT('UPDATE devices SET humidity = '
					"JSON_SET(humidity, '$.value', ",newData,", '$.time', '",timeS,"') "
					'WHERE iddevices = ',deviceID,';');
PREPARE stmt from @creation;
EXECUTE stmt;

#Also add the data to the temperature data table
INSERT INTO humidity(iddevices, humidity, time) VALUES(deviceID, newData, timeS);
